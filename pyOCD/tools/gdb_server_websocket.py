...
class GDBServerTool(object):
    ...
    def build_parser(self):
        ...
        parser.add_argument("-wh", "--websocket-host", dest="websocket_host", default="localhost",
                    help="Host on which websocket server is running.")
        parser.add_argument("-wp", "--websocket-port", dest="websocket_port", default=8081, type=int,
                    help="Port on which websocket server is running.")
        return parser
    ...
    def list_boards(self):
        ...
        try:
            all_mbeds = MbedBoard.getAllConnectedBoards(host=self.args.websocket_host,port=self.args.websocket_port,close=True, blocking=False)
            status = 0
            error = ""
    ...
    def run(self, args=None):
        ...
        if self.args.list_all == True:
            ...
            try:
                board_selected = MbedBoard.chooseBoardWS(
                    board_id=self.args.board_id,
                    target_override=self.args.target_override,
                    frequency=self.args.frequency,host=self.args.websocket_host,port=self.args.websocket_port)
